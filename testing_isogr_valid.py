...
for X_indices, y_label_t in tl.iterate.minibatches(X_teidx, 
  ...
  print('\RGB - Valid loss: {}, Valid acc: {}\n'.format(predict_loss, accu_value))
...
for X_indices, y_label_t in tl.iterate.minibatches(X_teidx, 
  ...
  print('\Depth - Valid loss: {}, Valid acc: {}\n'.format(predict_loss, accu_value))
...
video_list_d = './dataset_splits/valid_depth_list.txt'
f = open(video_list_d, 'r')
f_lines_d = f.readlines()
f.close()
f = open('valid_prediction_depth.txt', 'w')
print('%s: Escrevendo -  Depth' % datetime.now())
for idx, line in enumerate(f_lines_d):
  linetxt = '%s %s %d\n' %(line.split(' ')[0], line.split(' ')[1], prediction_values[idx].eval()+1)
  f.write(linetxt)
f.close()

#video_list_f = './dataset_splits/valid_flow_list.txt'
#f = open(video_list_f, 'r')
#f_lines_f = f.readlines()
#f.close()
#f = open('valid_prediction_flow.txt', 'w')
#print('%s: Escrevendo - Fluxo' % datetime.now())
#for idx, line in enumerate(f_lines_f):
#  linetxt = '%s %s %d\n' %(line.split(' ')[0], line.split(' ')[1], prediction_values[idx].eval()+1)
#  f.write(linetxt)
#f.close()

video_list_r = './dataset_splits/valid_rgb_list.txt'
f = open(video_list_r, 'r')
f_lines_r = f.readlines()
f.close()
f = open('libras_valid_prediction_rgb.txt', 'w')
print('%s: Escrevendo - RGB' % datetime.now())
...
sess.close()
print('%s: Fim de processamento' % datetime.now())
